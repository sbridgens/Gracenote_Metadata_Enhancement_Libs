    ...
    public class EnhancementDataValidator
    {
        ...
        public static bool ValidateVersionMajor(int? dbVersionMajor, bool isTvod)
        {
            ...
            if (dbVersionMajor > 0)
            {
                ...
                Log.Error($"Package for PAID: {paid} detected as an update but does not have a higher Version Major! Failing Enhancement.");
                return false;
            }

            Log.Error($"Package for PAID: {paid} detected as an update does not have a database entry! Failing Enhancement.");
            return false;
        }
