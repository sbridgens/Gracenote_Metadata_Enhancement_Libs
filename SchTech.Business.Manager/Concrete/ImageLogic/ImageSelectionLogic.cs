...
namespace SchTech.Business.Manager.Concrete.ImageLogic
{
    public class ImageSelectionLogic
    {
        ...
        /// Initialize log4net
        /// </summary>
        private readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(ImageSelectionLogic));
        ...
        private void SortAssets()
        {
            ...
            ApiAssetSortedList = IsLandscape
                ? ApiAssetList.OrderByDescending(w => w.width).ThenBy(h => h.height).ThenBy(i => i.identifiers.Any()).ToList()
                : ApiAssetList.OrderByDescending(h => h.height).ThenBy(w => w.width).ThenBy(i => i.identifiers.Any()).ToList();
        }
                        ...
                        foreach (var imageTier in category.ImageTier)
                        {
                            ...
                            foreach (var image in ApiAssetSortedList)
                            {
                                    ...
                                    PassesImageLogic(category, image, imageTier, image.tier, IsLandscape))
                                {
                                    ...
                                    {
                                        var match = Regex.Match(CurrentMappingData.GN_Images, $"(?m){ImageTypeRequired}:.*?.jpg");
