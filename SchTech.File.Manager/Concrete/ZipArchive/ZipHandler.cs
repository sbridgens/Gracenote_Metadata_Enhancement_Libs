...
namespace SchTech.File.Manager.Concrete.ZipArchive
{
    ...
    {
        private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(ZipHandler));
        ...
        {
            long entrySize = archiveEntry.Length;
            string outputFile = Path.Combine(OutputDirectory, archiveEntry.Name);
        ...
        {
            foreach (ZipArchiveEntry entry in archive.Entries.OrderByDescending(e => e.Length))
            {
        ...
        public bool ExtractItemFromArchive(string sourceArchive, string outputDirectory, bool extractAll, bool extractAdiOnly)
        {
            ...
            using (var archive = ZipFile.OpenRead(sourceArchive))
            {
                ...
                catch (Exception EIFA_Ex)
                {
                    ...
                    if (EIFA_Ex.InnerException != null)
                    {
                        Log.Error($"Inner Exception: {EIFA_Ex.InnerException.Message}");
                    }
